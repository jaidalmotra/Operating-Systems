...
int calculateTotalHeadMovements(int requests[], int size, int initialPosition) {
    ...
    while (true) {
        if (movingRight) {
            ...
            // Find the next request in the right direction
            for (int i = 0; i < size; i++) {
                if (requests[i] >= currentPosition && requests[i] < nextRequest) {
            ...
            int nextRequest = INT_MIN;
            int index = -1;

            // Find the next request in the left direction
            for (int i = 0; i < size; i++) {
                if (requests[i] <= currentPosition && requests[i] > nextRequest) {
                    nextRequest = requests[i];
                    index = i;
            ...
            if (index != -1) {
                ...
                break; // No more requests in the left direction
            }
        }
    }
